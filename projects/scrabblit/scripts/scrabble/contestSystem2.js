...
// checks if current player is the last on the array

if(nextContestantIndex == participantsArray.length) {

	// if so make nextContestantIndex zero
...
function updateContestant(contestantName) {
	...
	$.ajax({
		type: "POST",
		url: "../scripts/scrabble/contestantUpdater.php",
		data: {
...
// declare next player as a step ahead 

var nextPlayerIndex = participantsArray.indexOf(localStorage.getItem("scrabblitUsername")) + 1;

// checks if current player is the last on the array

if(nextPlayerIndex == participantsArray.length) {

	// if so make nextPlayerIndex zero
...
toast( "Awaiting results from the host" );
	...
	// the next player becomes the current player as soon as host clicks either the yes or no button
		...
		if(Number($("#currentPlayer").html()) == nextPlayerIndex) {
	...
	//				var previousSeeds = $("#rack").html();
					...
					// but if his response is yes replace seeds and deactivate isPlaying so his seeds can be replaced in the future and clear hostResult both in server and client side
					
				else if($("#hostResult").html() == "Yes") {
				alert("host agreed");
					replaceSeeds();
